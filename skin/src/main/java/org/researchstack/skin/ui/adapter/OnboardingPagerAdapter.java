...
public class OnboardingPagerAdapter extends PagerAdapter
{
    ...
    public Object instantiateItem(ViewGroup container, int position)
    {
        ...
        else if (item.getDetails().equals(OnboardingActivity.CONSENT_TAG))
        {
            ...
            emailButton.setOnClickListener(new View.OnClickListener()
            {
                ...
                public void onClick(View v)
                {
                    ...
                    emailIntent.putExtra(Intent.EXTRA_SUBJECT, R.string.rss_consent_doc);

                    try {
                        Uri uri = getConsentPdfUri();
                        emailIntent.putExtra(Intent.EXTRA_STREAM, uri);
                        inflater.getContext().startActivity(Intent.createChooser(emailIntent, "Send email..."));
                        LogExt.i(getClass(), "Sending consent PDF");
                    } catch (Exception e) {
                        Toast.makeText(inflater.getContext(), "Cannot send without external storage permission", Toast.LENGTH_LONG).show();
                    }
            ...
            viewButton.setOnClickListener(new View.OnClickListener()
            {
                ...
                public void onClick(View v)
                {
                    ...
                    Uri uri = null;
                    try {
                        uri = getConsentPdfUri();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    viewIntent.setDataAndType(uri, "application/pdf");

                    inflater.getContext().startActivity(Intent.createChooser(viewIntent, "Open PDF..."));
                    LogExt.i(getClass(), "Showing consent in PDF viewer");
                }
            });
    ...
    private Uri getConsentPdfUri() throws Exception
    {
        File externalCopy = ResourceManager.getInstance()
                .saveResourceToExternalStorage(inflater.getContext(), ResourceManager.getInstance().getConsentPDF());
        return Uri.fromFile(externalCopy);
    }
